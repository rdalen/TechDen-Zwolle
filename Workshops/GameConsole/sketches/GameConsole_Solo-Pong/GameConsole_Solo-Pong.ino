/*
  ...
  Name; GameConsole Solo Pong (Pong with ability to win)

  Functions;
  Buttons Left & Right to control the racket
  Button Up = start ball
...
byte target [3][COLOMNS];  //target LEDS
...
void setup() {
  ...
  Serial.begin(19200);
  Serial.println("GameConsole Solo pong");
  Serial.println("");     
...
//in Loop:
  ...
  //2 - Trigger drawFrame
  if (!blinkDisplay || demo) GraphicDriver::drawFrame(frameData); // write the frameData buffer into the LED's; 
...
// >>enter your code from here<<
  ...
  if (you_won) {
    ...
    you_won=false;
    ballPosX = NILL; // hide ball
    GraphicDriver::drawSprite(smile, 2000);  
    playSound(GOTIT);
    initTarget();
    if (demo){
      button=DOWN;
...
void initTarget(){ //fill the target LEDS
  for (uint8_t y=0; y<3; y++) {  //three upper rows
    for (uint8_t x=0; x<COLOMNS; x++) {
      target[y][x] = x;
    }     
  }
//  target[1][0]=NILL; //these leds can't be hit sometimes
//  target[1][2]=NILL;
//  target[1][4]=NILL;  
}
